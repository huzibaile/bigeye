...
from database.faction_model import RankWarRecord, Faction
...
def get_rw_report(rw_id: int):
    ...
    with mysql_db.session as session:
        ...
        if rw_info is None:
            ...
            rw_info = RankWarRecord.insert_or_update(rw_id, 'insert')
            # 更新数据出现错误
            ...
            # 更新rw_report数据
            RankWarRecord.insert_or_update(rw_id)
    ...
    faction = Faction.f
    attack_members, attack_logs = faction.get_attack_info(start_time, end_time)
    rw_report_dict = faction.construct_rw_report(attack_members)
    ...
    bonus_rule = RW_STRATEGY.get('bonus')
    ...
    for attack_log in attack_logs:
        ...
        if defender_faction == faction_id and attacker_faction in [enemy_faction_id, '']:
            ...
            continue
        # 4.3打中bonus
        if chain_num in bonus_rule and attacker_faction == faction_id:
            ...
            if result in ['Arrested', 'Attacked', 'Hospitalized', 'Mugged', 'Special']:
                # 判断是否打歪
                if chain_num >= bonus_rule.get('error_criteria') and defender_faction != enemy_faction_id:  # 打歪
                    # 打歪属于打野,这里只扣分,其他逻辑在打野处理
                    rw_report_dict.get(attacker_name)['respect_gain'] -= bonus_rule.get(chain_num)
                else:  # 打中
